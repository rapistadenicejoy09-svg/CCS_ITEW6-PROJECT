...
import { apiGetInstruction, apiUpdateInstruction, apiUploadInstructionFile } from '../lib/api'
...
export default function AdminEditMaterial() {
  ...
  useEffect(() => {
    ...
    const fetchMaterial = async () => {
      ...
      try {
        ...
        if (active && res.instruction) {
          const inst = res.instruction
          setFormData({
            title: inst.title || '',
            type: inst.type || 'curriculum',
            course: inst.course || '',
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem('authToken')
      await apiUpdateInstruction(token, id, {
        title: fd.title,
        type: fd.type,
        course: fd.course,
        subject: fd.subject,
      ...
      navigate(`/admin/instructions/${id}`)
    } catch (err) {
    ...
    <div className="module-page max-w-4xl mx-auto w-full">
      <div className="w-full space-y-5">
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="main-title font-extrabold text-[var(--text)]">Edit Material</h1>
            <p className="main-description text-[var(--text-muted)] mt-1">
              Update existing academic documents, courses, or attachments.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Link to={`/admin/instructions/${id}`} className="btn btn-secondary">← Cancel</Link>
            <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
          ...
          <div className="space-y-5">
            {/* Attachment Card */}
            <Card className="h-full border-[var(--accent)]/30 hover:border-[var(--accent)] transition-colors">
              ...
              {uploadMode === 'file' ? (
                <div
                  className={`flex flex-col items-center justify-center h-48 border-2 border-dashed rounded-xl transition-colors cursor-pointer group ${
                    uploadedFile || (fd.link && fd.link.startsWith('gridfs://'))
                ...
                >
                  ...
                  ) : fd.link && fd.link.startsWith('gridfs://') ? (
                    <>
                      <div className="w-10 h-10 rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center mb-3">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                      </div>
                      <p className="text-sm font-bold text-emerald-400">File attached</p>
                      <p className="text-[11px] text-[var(--text-muted)] mt-1">Click to replace existing file</p>
                    </>
                  ) : (
                    <>
                      <IconUpload />
                ...
                <div className="flex flex-col justify-center h-48 space-y-4">
                   ...
                   <div className="p-3 bg-[rgba(0,0,0,0.2)] border border-[var(--border-color)] rounded-lg">
                     <p className="text-xs text-[var(--text-muted)] leading-relaxed">
                       You can link directly to <span className="text-[var(--accent)] font-semibold">Google Drive</span>, <span className="text-[var(--accent)] font-semibold">OneDrive</span>, or <span className="text-blue-400 font-semibold">Dropbox</span> to replace the current attachment.
                     </p>
                   </div>
                </div>
              )}
